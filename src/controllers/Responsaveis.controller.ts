...
class ResponsaveisController {
  ...
  async listall(request: Request, response: Response): Promise<any> {
    ...
    try {
      ResponsaveisController.logs.sendLog(LogsService.SYSTEM, LogsService.MODULE.ASSUNTOS, LogsService.TRANSACTION.LISTAR, request.user, request.user.unidadeUsuario.unit_name, request.body);
